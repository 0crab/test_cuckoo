...
int main(int argc, char **argv) {
    if (argc > 7) {
        thread_number = std::atol(argv[1]); //16
        key_range = std::atol(argv[2]);  //1000000000
        total_count = std::atol(argv[3]); //10000000
        timer_range = std::atol(argv[4]);   //120
        skew = std::atof(argv[5]);  //0
        updatePercentage = std::atoi(argv[6]); //100
        ereasePercentage = std::atoi(argv[7]); //0
